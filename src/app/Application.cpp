...
Application::Application() : window(), camera(glm::vec3(10.f, 10.f, 10.f)), light(glm::vec3(200.f, 200.f, -200.f)) {
    ...
    while (!glfwWindowShouldClose(window.getGLFWwindow())) {
        ...
        if (vdbFile) {
            ...
            glm::mat4 projection = glm::perspective(
                glm::radians(camera.getFOV()), float(window.getWidth()) / float(window.getHeight()), 0.1f, 100.0f);
